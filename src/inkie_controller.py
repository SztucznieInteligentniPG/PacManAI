...
class InkieController(Controller):
    ...
    def update(self, world: World):
        ...
        if self.timer >= self.tickRate:
            ...
            helper = world.getPositionInDirection(self.pacman.worldPosition,self.pacman.direction)
            helper = world.getPositionInDirection(helper,self.pacman.direction)
            blinkie = world.getBlinkiePosition()
            vector = Vector2Int(helper.x-blinkie.x,helper.y-blinkie.y)
            target = Vector2Int(abs(helper.x - vector.x), abs(helper.y - vector.y))
            ...
            if world.gameState == GameState.RUNNING_CHAOS:
                for position in positions:
                    ...
                    for obstacle in obstacles:
                        if isinstance(obstacle,Wall):
                            point = 10000
                    point += world.countDistance(Vector2Int(0,0),position)
                    points.append(point)
                direction_index = points.index(min(points))
                self.direction = Direction(direction_index)
