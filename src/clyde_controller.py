...
class ClydeController(Controller):
    ...
    def update(self, world: World):
        ...
        if self.timer >= self.tickRate:
            ...
            radius = world.countDistance(self.pacman.worldPosition,grandPosition)
            ...
            points = []
            if world.gameState == GameState.RUNNING and radius>8:
            ...
            if world.gameState == GameState.RUNNING_CHAOS or radius <= 8:
                for position in positions:
                    obstacles = world.getEntities(position)
                    point = 0
                    for obstacle in obstacles:
                        if isinstance(obstacle,Wall):
                            point = 10000
                    point += world.countDistance(Vector2Int(20,20),position)
                    points.append(point)
                direction_index = points.index(min(points))
                self.direction = Direction(direction_index)
